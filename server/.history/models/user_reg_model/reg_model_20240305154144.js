    ...
    try{
        ...
        connectDB.queryObject(sql_reg_commands ,(error, result) => {
            if (error) {
                ...
                console.log(result);
                if(result == true){
                    //making table for each user base on the name of user "user_name"
                    var creating_table_command = `CREATE TABLE IF NOT EXISTS ${user_name} (keys INTEGER NOT NULL , food_name VARCHAR(50) NOT NULL , food_id INTEGER NOT NULL, quantity INTEGER NOT NULL , weight INTEGER NOT NULL, expDate VARCHAR(8) NOT NULL , UNIQUE (food_id , key), PRIMARY KEY (food_id , key))` ; 
