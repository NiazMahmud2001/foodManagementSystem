    ...
    try{
        ...
        connectDB.queryObject(sql_reg_commands ,(error, result) => {
            if (error) {
                ...
                console.log(result);
                if(result == true){
                    //making table for each user base on the name of user "email"
                    var creating_table_command = `CREATE TABLE IF NOT EXISTS ${phone_number} (keys INT NOT NULL , food_name VARCHAR(50) NOT NULL , food_id INT NOT NULL, food_name VARCHAR(50) NOT NULL , quantity INT NOT NULL , weight INT NOT NULL, expDate VARCHAR(8) NOT NULL , UNIQUE (food_id , key), PRIMARY KEY (food_id , key))` ; 
